...
public class S_openServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		String url = "/front-store-end/storeData/listOneOpenHour.jsp";
		...
		if ("getOne_For_Display".equals(action)) {
			...
			try {
				String storeno1 = req.getParameter("storeno");
				System.out.println("getOneforDisplay"+storeno1);
				S_openService openSvc = new S_openService();
				List<S_openVO> openlist = openSvc.getOneOpen(storeno1);
				if (openlist == null) {
					errorMsgs.add("查無資料");
				}
				...
				//模擬店家已登入狀態		
				session.setAttribute("storeno", storeno1);
		...
		if ("delete".equals(action)) {
			...
			System.out.println("delete"+storeno);	
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-store-end/storeData/listOneOpenHour.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				RequestDispatcher success = req.getRequestDispatcher("/front-store-end/storeData/listOneOpenHour.jsp");
				success.forward(req, res);
