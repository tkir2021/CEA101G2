...
//				}
				...
				String mailMsg = "點擊以下連結啟用帳號：http://localhost:8081/CEA101G2/member/member.do?action=verify&verifyCode="
						+ verify.insertCode(mem_acct) + "&mem_acct=" + mem_acct;
			...
			try {
				...
				if (!(mem_DateVO.getMem_img() == null)) {
					...
					InputStream in = getServletContext()
							.getResourceAsStream("/front-customer-end/member/img/logo_top.png");
		...
		if ("updateAuth".equals(action)) { // 來自update_mem_input.jsp的請求
			...
			try {
				...
				errorMsgs.add("修改資料失敗:" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/member/mem_back.jsp");
				failureView.forward(req, res);
			}
		}
